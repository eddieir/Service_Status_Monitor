...
def get_apps(app_id=None):
    if app_id is None:
        ...
        rs = query_db('select * from Applications where id = ?',
                      [app_id], one=True)
        return jsonify({"code": 200, 'data': rs})
...
def updateGPUServer(_id, address):
    with app.app_context():
        ...
        machines_status[_id] = {"data": status, "update_time": int(
...
def getGPUServerInfo():
    ...
    gpu_server_list = list(
        filter(lambda server: server['gpu'] == 1, server_list))
...
def check():
    ...
    app_list = list(filter(_filter, requests.get(
        server_base + '/apps').json()['data']))
